...
# copybara:strip_begin(path variables)
# INGESTION_SCRIPTS_PATH = "google3.third_party.chronicle.ingestion_scripts."
# sys.modules[f"{INGESTION_SCRIPTS_PATH}common"] = mock_common
# sys.modules[f"{INGESTION_SCRIPTS_PATH}common.utils"] = mock_utils
# sys.modules[f"{INGESTION_SCRIPTS_PATH}common.ingest_v1"] = mock_ingest_v1
# INGESTION_SCRIPTS_PATH = INGESTION_SCRIPTS_PATH + "cyware_threat_intelligence_exchange."
...
# from google3.third_party.chronicle.ingestion_scripts.cyware_threat_intelligence_exchange import constant
# from google3.third_party.chronicle.ingestion_scripts.cyware_threat_intelligence_exchange import cyware_client
# from google3.third_party.chronicle.ingestion_scripts.cyware_threat_intelligence_exchange.exception_handler import (
#     CywareCTIXException,
#     RunTimeExceeded,
# )
# copybara:strip_end_and_replace_begin
...
class TestGetStartTime(unittest.TestCase):
    ...
    def test_get_start_time_with_lookback(self, mock_log):
        ...
        result = client.get_start_time("7")
    ...
    def test_get_start_time_default(self, mock_log):
        ...
        result = client.get_start_time(None)
        self.assertIsInstance(result, int)
    ...
    def test_get_start_time_invalid_lookback(self, mock_log):
        ...
        result = client.get_start_time("invalid")
        self.assertIsInstance(result, int)
    ...
    def test_get_start_time_negative_days(self, mock_log):
        ...
        result = client.get_start_time("-5")
        self.assertIsInstance(result, int)
...
class TestExtractIOCValues(unittest.TestCase):
    ...
    def test_extract_ioc_values_success(self, mock_log):
        ...
        result = client.extract_ioc_values(indicators)
    ...
    def test_extract_ioc_values_empty(self, mock_log):
        ...
        result = client.extract_ioc_values([])
...
class TestFetchIndicatorData(unittest.TestCase):
    ...
    def test_fetch_runtime_exceeded_exception(
        ...
        with patch.object(
            client, "_get_checkpoints_and_timestamps"
        ) as mock_ckpt:
...
class TestCtixRestApi(unittest.TestCase):
    ...
    @patch(f"{INGESTION_SCRIPTS_PATH}cyware_client.requests.request")
    @patch(f"{INGESTION_SCRIPTS_PATH}cyware_client.utils.cloud_logging")
    def test_ctix_rest_api_return_dict_retry_false(
...
class TestParseAndHandleResponse(unittest.TestCase):
    ...
    @patch(f"{INGESTION_SCRIPTS_PATH}cyware_client.utils.cloud_logging")
    def test_parse_200_success(self, mock_log):
        """Test parsing 200 success response."""
        client = cyware_client.CTIXClient(
            base_url="https://example.com",
            access_id="id",
            secret_key="key",
    ...
    @patch(f"{INGESTION_SCRIPTS_PATH}cyware_client.utils.cloud_logging")
    def test_parse_401_unauthorized(self, mock_log):
    ...
    @patch(f"{INGESTION_SCRIPTS_PATH}cyware_client.utils.cloud_logging")
    def test_parse_403_forbidden(self, mock_log):
    ...
    @patch(f"{INGESTION_SCRIPTS_PATH}cyware_client.utils.cloud_logging")
    def test_parse_429_rate_limit(self, mock_log):
    ...
    @patch(f"{INGESTION_SCRIPTS_PATH}cyware_client.utils.cloud_logging")
    def test_parse_500_server_error(self, mock_log):
    ...
    @patch(f"{INGESTION_SCRIPTS_PATH}cyware_client.utils.cloud_logging")
    def test_parse_other_error(self, mock_log):
        """Test parsing other error status codes."""
        client = cyware_client.CTIXClient(
            base_url="https://example.com",
            access_id="id",
            secret_key="key",
...
class TestMakeApiCall(unittest.TestCase):
    ...
    def test_make_api_call_success(self, mock_log):
        ...
        with patch.object(client, "_ctix_rest_api") as mock_api:
            with patch.object(
                client, "_parse_and_handle_response"
            ) as mock_parse:
...
class TestEnrichmentMethods(unittest.TestCase):
    ...
    def test_process_enrichment_chunk_api_error(self, mock_log):
        ...
        with patch.object(client, "make_api_call") as mock_api:
            ...
            with patch.object(client, "_save_error_checkpoint") as mock_save:
                with self.assertRaises(CywareCTIXException):
                    client._process_enrichment_chunk(
                        0, indicators, 0, 1000, 2000, 1
                    )
    ...
    def test_ingest_without_enrichment(self, mock_log):
        ...
        with patch.object(client, "_ingest_indicators") as mock_ingest:
            ...
            result = client._ingest_without_enrichment(
    ...
    @patch(f"{INGESTION_SCRIPTS_PATH}cyware_client.utils.cloud_logging")
    def test_enrich_and_ingest_by_chunks(self, mock_log):
        ...
        indicators = [
            {"sdo_name": f"ioc{i}", "ctix_modified": 1000 + i}
            for i in range(10)
        ]

        with patch.object(client, "extract_ioc_values") as mock_extract:
...
class TestCheckpointsValidationEdgeCases(unittest.TestCase):
    ...
    @patch(f"{INGESTION_SCRIPTS_PATH}cyware_client.time.time")
    @patch(f"{INGESTION_SCRIPTS_PATH}cyware_client.utils.cloud_logging")
    def test_from_ge_to_timestamp(
        self, mock_log, mock_time, mock_get, mock_set
    ):
    ...
    def test_valid_page_number_resume(self, mock_log, mock_get):
        """Test resuming with valid positive page number.

        Covers lines 500-504.
        """
...
class TestCoverRemainingLines(unittest.TestCase):
    ...
    def test_line_530_extract_ioc_values_called(self, mock_log, mock_get):
        ...
        # This should internally use extract_ioc_values
...
class TestFetchIndicatorsByLabels(unittest.TestCase):
    ...
    def test_no_label_provided(self, mock_log, mock_get, mock_set, mock_clear):
        ...
        has_warning = any(
            "No saved result set list provided" in str(call)
