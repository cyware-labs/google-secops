...
sys.modules[f"{INGESTION_SCRIPTS_PATH}common"] = MagicMock()
sys.modules[f"{INGESTION_SCRIPTS_PATH}common.utils"] = MagicMock()
...
# from google3.third_party.chronicle.ingestion_scripts.cyware_threat_intelligence_exchange import exception_handler
...
class TestExceptionHandlerDecorator(unittest.TestCase):
  ...
  def test_decorator_with_general_exception(self, mock_logging):
    ...
    def general_error_function():
      raise Exception("General error")  # pylint: disable=broad-exception-raised
