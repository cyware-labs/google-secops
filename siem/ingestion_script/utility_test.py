...
mock_env_constants.ENV_CHRONICLE_SERVICE_ACCOUNT = "ENV_CHRONICLE_SERVICE_ACCOUNT"
...
# sys.modules[f"{INGESTION_SCRIPTS_PATH}common.env_constants"] = mock_env_constants
...
# from google3.third_party.chronicle.ingestion_scripts.cyware_threat_intelligence_exchange import utility
...
class TestGetEnvironmentVariable(unittest.TestCase):
  ...
  def _make_env_fetch_validator(self, expected_default, *, expected_required=False):
...
class TestSetLastCheckpoint(unittest.TestCase):
  ...
  @patch(f"{INGESTION_SCRIPTS_PATH}utility.google.cloud.resourcemanager_v3.ProjectsClient")
  @patch(f"{INGESTION_SCRIPTS_PATH}utility.utils.get_env_var")
  @patch(f"{INGESTION_SCRIPTS_PATH}utility.utils.cloud_logging")
...
class TestClearCheckpointIfExists(unittest.TestCase):
  ...
  def test_clear_checkpoint_exists(self, mock_log, mock_get, mock_set):
    ...
    mock_set.assert_called_once_with("tenant1", "test-bucket", "test_key", None)
...
class TestSetLastCheckpointEdgeCases(unittest.TestCase):
  ...
  def test_set_checkpoint_preserves_other_tenants(
    ...
    @patch(f"{INGESTION_SCRIPTS_PATH}utility.utils.get_env_var")
    def test_get_environment_variable_label_name_with_escapes(
        self, mock_get_env
    ):
...
class TestAcquireProcessLock(unittest.TestCase):
  ...
  def test_acquire_lock_recent_run_returns_false(
    ...
    warning_messages = [str(call.args[0]) for call in mock_log.call_args_list]
    self.assertTrue(any("Another process is already running" in msg for msg in warning_messages))
  ...
  def test_acquire_lock_invalid_timestamp_returns_false(
    ...
    mock_set_checkpoint.assert_not_called()
    warning_messages = [str(call.args[0]) for call in mock_log.call_args_list]
    self.assertTrue(any("Invalid last_run_initiation_time" in msg for msg in warning_messages))
  ...
  def test_acquire_lock_missing_last_run_returns_false(
    ...
    mock_set_checkpoint.assert_not_called()
    warning_messages = [str(call.args[0]) for call in mock_log.call_args_list]
    self.assertTrue(any("Lock status: true" in msg for msg in warning_messages))
  ...
  def test_acquire_lock_propagates_unexpected_errors(
    ...
    current_time = self._setup_time(mock_time, 9000.0)
    mock_get_checkpoint.return_value = None
    ...
    self.assertTrue(any("Failed to acquire process lock" in msg for msg in error_messages))
