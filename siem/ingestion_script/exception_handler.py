...
class CywareCTIXException(Exception):
    """Custom exception for Cyware CTIX API errors."""
...
class RunTimeExceeded(Exception):
    """Exception raised when execution time limit is exceeded."""
        ...
        def inner(*args, **kwargs):
            ...
            try:
                ...
                utils.cloud_logging(
                    ...
                    "Error = {1}\n"
                    "Traceback: {2}".format(function_name, repr(e), traceback.format_exc()),
                    severity="ERROR",
                )
                result["retry"] = True
                result["error"] = (
                    "API call failed. Failed due to connection "
                    "timeout.{0}".format(constant.CHECK_LOGS_FOR_MORE_DETAILS)
                )
                ...
                result["error"] = (
                    "API call failed. Failed due to too many "
                    "redirects.{0}".format(constant.CHECK_LOGS_FOR_MORE_DETAILS)
                )
                ...
                result["error"] = (
                    "API call failed. Failed due to SSL error.{0}".format(
                        constant.CHECK_LOGS_FOR_MORE_DETAILS
                    )
                )
