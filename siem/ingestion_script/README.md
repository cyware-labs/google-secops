...
- Create a zip file with the contents of the following files:
  ...
  - Download the contents of the secops-ctix-siem ingestion script.
...
This section provides complete step-by-step CLI commands to deploy the Cyware
Intel Exchange integration without using the automated deployment script.
...
export GCS_BUCKET_NAME="cyware-ctix-bucket-${PROJECT_ID}"
export LOCAL_ZIP_PATH="./cyware_ctix_function.zip"  # Path to your uploaded zip file

# Cloud Function Configuration
export CF_NAME="cyware-ctix-secops-function"
export SCHEDULER_NAME="cyware-ctix-secops-scheduler"
...
# Cyware CTIX Configuration
export CYWARE_BASE_URL="https://your-ctix-instance.com"
export CYWARE_TENANT_NAME=""  # Optional
export CYWARE_ENRICHMENT_ENABLED="false"
export CYWARE_INDICATOR_LOOKBACK_DAYS="30"
export CYWARE_SAVED_RESULT_SET_NAME="your-saved-result-set-name"  # Required

# Credentials (will be stored in Secret Manager)
export CYWARE_ACCESS_ID_VALUE="your-access-id"
export CYWARE_SECRET_KEY_VALUE="your-secret-key"
...
```bash
# Create and store Cyware Access ID
echo -n "$CYWARE_ACCESS_ID_VALUE" | gcloud secrets create cyware-access-id \
  --replication-policy="automatic" \
  --data-file=- \
  --project=$PROJECT_ID

export CYWARE_ACCESS_ID_SECRET=$(gcloud secrets versions describe latest \
  --secret="cyware-access-id" \
  --project=$PROJECT_ID \
  --format='value(name)')

echo "Cyware Access ID Secret: $CYWARE_ACCESS_ID_SECRET"

# Create and store Cyware Secret Key
echo -n "$CYWARE_SECRET_KEY_VALUE" | gcloud secrets create cyware-secret-key \
  --replication-policy="automatic" \
  --data-file=- \
  --project=$PROJECT_ID

export CYWARE_SECRET_KEY_SECRET=$(gcloud secrets versions describe latest \
  --secret="cyware-secret-key" \
  --project=$PROJECT_ID \
  --format='value(name)')

echo "Cyware Secret Key Secret: $CYWARE_SECRET_KEY_SECRET"
...
gsutil cp $LOCAL_ZIP_PATH "gs://$GCS_BUCKET_NAME/cyware_ctix_function.zip"

echo "Cloud Function ZIP uploaded to gs://$GCS_BUCKET_NAME/cyware_ctix_function.zip"
```
...
ENV_VARS="${ENV_VARS},CYWARE_ACCESS_ID=${CYWARE_ACCESS_ID_SECRET}"
ENV_VARS="${ENV_VARS},CYWARE_SECRET_KEY=${CYWARE_SECRET_KEY_SECRET}"
ENV_VARS="${ENV_VARS},CYWARE_BASE_URL=${CYWARE_BASE_URL}"
ENV_VARS="${ENV_VARS},CYWARE_ENRICHMENT_ENABLED=${CYWARE_ENRICHMENT_ENABLED}"
ENV_VARS="${ENV_VARS},CYWARE_INDICATOR_LOOKBACK_DAYS=${CYWARE_INDICATOR_LOOKBACK_DAYS}"
ENV_VARS="${ENV_VARS},CYWARE_SAVED_RESULT_SET_NAME=${CYWARE_SAVED_RESULT_SET_NAME}"
...
[ -n "$CYWARE_TENANT_NAME" ] && ENV_VARS="${ENV_VARS},CYWARE_TENANT_NAME=${CYWARE_TENANT_NAME}"
...
gcloud functions deploy $CF_NAME \
  ...
  --runtime=python312 \
  --region=$REGION \
  --source="gs://$GCS_BUCKET_NAME/cyware_ctix_function.zip" \
  --entry-point=main \
...
gcloud secrets list --project=$PROJECT_ID --filter="name:cyware OR name:chronicle"
...
| CYWARE_ACCESS_ID | Copied resource name value of Access ID from secret manager.<br> Generate Access ID and Secret Key from the Cyware CTIX platform's API settings section. | Yes | string | - | Yes |
| CYWARE_SECRET_KEY | Copied resource name value of Secret Key from secret manager.<br> Generate Access ID and Secret Key from the Cyware CTIX platform's API settings section. | Yes | string | - | Yes |
| CYWARE_BASE_URL | Base URL of your Cyware CTIX instance (e.g., https://your-ctix-instance.com). | Yes | string | - | No |
| CYWARE_TENANT_NAME | Tenant name for multi-tenant Cyware CTIX deployments. | No | string | - | No |
...
| CYWARE_SAVED_RESULT_SET_NAME | Filter indicators by specific label name in Cyware CTIX. | Yes | string | - | No |

### Creating zip of the cloud function
Create a zip file of the cloud function with the contents of the
following files:

1. Contents of the ingestion script (i.e. `secops-ctix-siem`)
2. `common` directory
...
1. Navigate to the bucket and open the bucket created for the Cyware CTIX in [these](#create-a-gcp-bucket) steps. Upload the created cloud function [zip](#creating-zip-of-the-cloud-function) file in the bucket.
...
minutes. If the execution time of the Cloud Function exceeds timeout then there
are chances that the complete data won't be ingested in the Google SecOps.
* The rate limit for a Cyware CTIX account depends on the user's subscription
...
* The comma-separated format for CYWARE_SAVED_RESULT_SET_NAME is not supported during manual Cloud Function deployment, as commas are treated as special characters while deploying the function through CLI. To use comma-separated values, please deploy the function using the provided deployment script.
...
- [Cyware CTIX Documentation](https://www.cyware.com/ctix)
