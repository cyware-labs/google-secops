...
**Note:** When using the manual deployment flow, avoid comma-separated values for `CYWARE_SAVED_RESULT_SET_NAME`. Use the provided deployment script if you need to supply comma-separated saved result set names.
...
**Note:** Ensure that the resource Name format matches the following structure: `projects/{project_id}/secrets/{secret_id}/versions/{version_id}`.
...
- The rate limit for a Cyware Intel Exchange account depends on the user's subscription
